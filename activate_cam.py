...
while(True):
    ...
    curr_temp = (np.array(temps).mean())
    hum_reading = sense.get_humidity()
    hums.pop(0)
    hums.append(hum_reading)
    curr_hum = (np.array(hums).mean())
    time.sleep(0.1)
    if (count % 20 == 0):
        if (old_temp is None):
            old_temp = curr_temp
        else:
            if (abs(old_temp - curr_temp) > 1):
                break
    if (count % 20 == 0):
        if (old_hum is None):
            old_hum = curr_hum
        else:
            if (abs(old_hum - curr_hum) > 1):
                break
    count += 1
...
sense = SenseHat()
def gen_audio_message():
    if ("TTSaudio.mp3" not in os.listdir()):
...
while True:
    ...
    faces = eyeCascade.detectMultiScale(frameGray,1.3,3)
    print(faces)
    for face in faces:
        x,y,w,h = face
